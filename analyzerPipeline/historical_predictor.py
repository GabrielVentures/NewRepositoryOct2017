...
cur.execute("""SELECT id from successful_cohorts WHERE seq_sales_prediction is NULL""")
rows = cur.fetchall()
ids = set()
for row in rows:
    cohort_id = row[0]
    ...
    statement = "UPDATE successful_cohorts SET seq_sales_prediction=" + str(next_quarter_prediction) + " WHERE seq_sales_prediction is NULL AND id=" + str(cohort_id)
    print(statement)
    cur.execute(statement)
    conn.commit()
    print('Done')
