...
def get_cohort_by_id(cohort_id):
    statement = """SELECT panelist_id from panelist_cohorts WHERE cohort_id=""" + str(cohort_id)
    print statement
...
RETAILERS = list(actual_results_mapping.keys())
# Retrieve Real Data
...
def yy_sales_growth(test_data, retailer):
    ...
    print 'prediction for 3Q2017', prediction
    ...
    result['prediction_error_for_3Q2017'] = sum_of_diff
    result['prediction_3Q2017'] = prediction
    return result


cur.execute("""SELECT id, retailer from successful_cohorts WHERE yy_sales_prediction is NULL""")
rows = cur.fetchall()
ids = set()
print(rows)
for row in rows:
    cohort_id = row[0]
    retailer = row[1]
    print 'cohort_id', cohort_id, retailer
    try:
        ...
        # pp.pprint(b)
        # pp.pprint(actual_results_mapping[RETAILER])
        result = yy_sales_growth(test_data, RETAILER)
        sum_of_diff = result['prediction_error_for_3Q2017']
        prediction = result['prediction_3Q2017']

        statement = "UPDATE successful_cohorts SET yy_sales_prediction=" + str(prediction) + ", yy_sales_error=" + str(sum_of_diff) + " WHERE yy_sales_prediction is null AND id=" + str(cohort_id)
        print(statement)
        cur.execute(statement)
        conn.commit()
    except Exception, e:
